...
We will see a quick setup to configure the ``tool_chrome`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_chrome`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_chrome`` formulas under ``/srv/salt/tool_chrome/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_chrome/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_chrome/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_chrome/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_chrome/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_chrome`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_chrome`` formulas under ``/srv/salt/tool_chrome/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_chrome/parameters/roles
...
We create a configuration for the role ``tool_chrome/server`` in ``/srv/salt/tool_chrome/parameters/roles/tool_chrome/server.yaml``:
...
We create another configuration for the role ``tool_chrome/client`` in ``/srv/salt/tool_chrome/parameters/roles/tool_chrome/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_chrome/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_chrome/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_chrome/parameters/dns:domain/  '/srv/salt/tool_chrome/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-chrome-as-json:
      test.nop:
        - name: {{ chrome | json }}
...
.. code-block:: sls
    ...
    chrome-config-file-file-managed:
      file.managed:
        - name: {{ chrome.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='chrome-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``chrome`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ chrome['winner'] }}
